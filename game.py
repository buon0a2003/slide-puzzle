...
class Application(tk.Frame):
	def __init__(self, master=None):
		...
		self.imgType.set('bac_ho')
		self.numMoves = 0
		self.firstMove = True
		self.timer_id = None

		self.imgDict = {'bác hồ':bac_ho_list, 'lăng bác':lang_bac_list, 'nhà sàn':nha_san_list, 'bến nhà rồng': ben_nha_rong_list}
		self.solDict = {'bác hồ':bac_ho_sol, 'lăng bác':lang_bac_sol, 'nhà sàn':nha_san_sol, 'bến nhà rồng': ben_nha_rong_sol}
	...
	def draw_header(self):
		...
		self.options = ttk.OptionMenu(self.header, self.imgType, 'bác hồ', *self.imgDict.keys())
...
if __name__ == '__main__':
	root = tk.Tk()
	root.title('Puzzle tư tưởng hcm')
	...
	bac_ho_list = [PhotoImage(file=f'Images/bac_ho/img{index}.png') for index in range(1,17)]
	lang_bac_list = [PhotoImage(file=f'Images/lang_bac/img{index}.png') for index in range(1,17)]
	nha_san_list = [PhotoImage(file=f'Images/nha_san/img{index}.png') for index in range(1,17)]
	ben_nha_rong_list = [PhotoImage(file=f'Images/ben_nha_rong/img{index}.png') for index in range(1,17)]

	bac_ho_sol = PhotoImage(file='Images/bac_ho_resized.png')
	lang_bac_sol = PhotoImage(file='Images/lang_bac_resized.png')
	nha_san_sol = PhotoImage(file='Images/nha_san_resized.png')
	ben_nha_rong_sol = PhotoImage(file='Images/ben_nha_rong_resized.png')
